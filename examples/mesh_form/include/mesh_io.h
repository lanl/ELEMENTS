    ...
    int Pn_order)
{
    ...
    if(Pn_order == 0) {
        is_linear = true;
        std::cout<<"Pn order is 0, using linear elements"<<std::endl;
        Pn_order = 1;
    }
    ...
    node_coords = DCArrayKokkos<double>(num_nodes, num_dim, "node_coordinates");
               ...
               double time_value = 0.0)
{
    ...
    const int num_point_vec_vars = 3;
    ...
    const char point_scalar_var_names[num_point_scalar_vars][15] = {
        "rank_id", "elems_in_node", "global_node_id", "distance_facet"
    };

    const char point_vec_var_names[num_point_vec_vars][15] = {
        "pos", "normal", "closest_point"
    };
    ...
    for (size_t elem_gid = 0; elem_gid < num_elems; elem_gid++) {
        ...
        for (int dim = 0; dim < num_dims; dim++) {
            elem_vec_fields(elem_gid, 0, dim) = gauss_point.position.host(elem_gid, dim);
        }
    }
    ...
    for (size_t node_gid = 0; node_gid < num_nodes; node_gid++) {
        ...
        // vector field, var 1
        for (int dim = 0; dim < num_dims; dim++) {
            vec_fields(node_gid, 1, dim) = node.normal.host(node_gid, dim);
        }

        for (int dim = 0; dim < num_dims; dim++) {
            vec_fields(node_gid, 2, dim) = node.closest_facet_point.host(node_gid, dim);
        }
    ...
    fprintf(vtu_file, "        <DataArray type=\"Float64\" Name=\"TimeValue\" NumberOfTuples=\"1\" format=\"ascii\">\n");
    fprintf(vtu_file, "          %f\n", time_value);
    ...
    for (int var = 0; var < num_point_vec_vars; var++) {
        fprintf(vtu_file, "        <DataArray type=\"Float32\" Name=\"%s\" NumberOfComponents=\"%d\" format=\"ascii\">\n",
                point_vec_var_names[var], (int)num_dims);
        for (size_t node_gid = 0; node_gid < num_nodes; node_gid++) {
            for (int dim = 0; dim < num_dims; dim++) {
    ...
    for (int var = 0; var < num_cell_vec_vars; var++) {
        fprintf(vtu_file, "        <DataArray type=\"Float32\" Name=\"%s\" NumberOfComponents=\"%d\" format=\"ascii\">\n",
                cell_vec_var_names[var], (int)num_dims);
        for (size_t elem_gid = 0; elem_gid < num_elems; elem_gid++) {
            // TODO: Populate cell vector field data from appropriate source
            for (int dim = 0; dim < num_dims; dim++) {
                fprintf(vtu_file, "          %f\n", 
                    gauss_point.position.host(elem_gid, dim));
            }
        }
        fprintf(vtu_file, "        </DataArray>\n");
    }
    ...
    if (rank == 0) {
        ...
        fprintf(pvtu_file, "    <FieldData>\n");
        fprintf(pvtu_file, "      <DataArray type=\"Float64\" Name=\"TimeValue\" NumberOfTuples=\"1\" format=\"ascii\">\n");
        fprintf(pvtu_file, "        %f\n", time_value);
        fprintf(pvtu_file, "      </DataArray>\n");
        fprintf(pvtu_file, "    </FieldData>\n");
