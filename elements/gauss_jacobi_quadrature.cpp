...
void compute_gauss_jacobi_quadrature_rule(
  ...
  if (n > 1) {
    ...
    CArray<NumType> diag(n);
    CArray<NumType> subdiag(n-1);
    ...
    CArray<NumType> eigvals(n);
    CArray<NumType> eigvecs(n,n);
    ...
    // the gamma function
    RealNumber mu0 = pow(2.0, common::real(alpha + beta + 1.0))
    ...
    for (int j = 0; j < n; j++) {
      ...
      RealNumber q_0j = eigvecs(j,0);  // Note: eigvecs are along rows
