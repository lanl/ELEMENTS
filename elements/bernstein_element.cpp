#include "bernstein_element.h"
#include "bernstein_polynomials.h"


template <typename NumType>
BernsteinElement<NumType>::BernsteinElement(const SizeType order) : Np( order) { 
  N = Np + 1;
  Ne = std::pow(N, Nd);
...
template <typename NumType>
BernsteinElement<NumType>::~BernsteinElement() { }


template <typename NumType>
NumType BernsteinElement<NumType>::eval_basis(const SizeType I, const NumType *X) { 
...
/* Evaluate gradient of local function approximation, which is formed by the sum of the products of tensor-product Bernstein basis functions and the provided coefficients, at specified coordinates. */
template <typename NumType>
void BernsteinElement<NumType>::eval_grad_approx(const NumType *c, const NumType *X, NumType *grad_f) { 
  for (int l = 0; l < Nd; l++) { 
    for (int k =0; k < N; k++) {
      for (int j =0; j < N; j++) {
        //Collapse first dimension into coefficients for second dimension
...
template <typename NumType>
void BernsteinElement<NumType>::eval_jac(const NumType *x, const NumType *y,
...
template <typename NumType>
NumType BernsteinElement<NumType>::eval_det_jac(const NumType *x, const NumType *y,
         const NumType *z, const NumType *X) {
    NumType J[9];
...
template class BernsteinElement<Real>;
template class BernsteinElement<Complex>;
