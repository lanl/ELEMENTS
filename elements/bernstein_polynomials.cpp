...
namespace bernstein {
  ...
  NumType eval( const int n, const int v, const NumType X) {
    ...
    if ( n == 0 && v == 0 ) return 1;
    if ( n < v ) return 0;
    return 0.5*((1-double(X))*eval(n-1, v, X) + (1+double(X))*eval(n-1, v-1, X)); 
  };
  template <typename NumType>
  NumType eval_der( const int n, const int v, const NumType X) {
    ...
    return 0.5*( (1-double(X))*eval_der(n-1,v, X) + (1+double(X))*eval_der(n-1, v-1, X) + eval(n-1, v-1, X) - eval(n-1, v, X) ) ;
  };
