...
struct TestParams {
  ...
  TestParams(SizeType order) {
    ...
    c = new NumType[elem->Nve];
    for (SizeType i = 0; i < elem->Nve; i++) {
      c[i] = Real(rand())/RAND_MAX;
    }

    // Select a coordinates between -1 and 1
    ...
    SizeType I = std::round((Nv - 1)*Real(rand())/Real(RAND_MAX));
    SizeType J = std::round((Nv - 1)*Real(rand())/Real(RAND_MAX));
    SizeType K = std::round((Nv - 1)*Real(rand())/Real(RAND_MAX));
...
bool test1(TestParams<Real> &p) {
  ...
  for (SizeType i = 0; i < p.elem->Nve; i++) {
